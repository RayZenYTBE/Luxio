...
module.exports.run = async (client, message, args) => {
   ...
   await db.delete(`points_${message.guild.id}_${user.id}`);
    user.send(`Your all mod points are reseted by ${message.author.username} from ${message.guild.name}`)

    let resetted = new Discord.MessageEmbed()
      .setColor('GOLD')
      .setAuthor('Moderator Points Resetted')
